...
table1(~age_cat + sex  + race_ethnicity + Yost_ACS_2006_2010_quintile + In_Medicaid_Flag  + stage + fumo  + can_type + can_dead2|mcont66, droplevels = TRUE, seer0to19 %>% filter(Agerecodewithsingleages_and_100 !=18 ))


table1(~mcont66|age_cat, droplevels = TRUE, seer0to19%>% filter(Agerecodewithsingleages_and_100 <18 ))
```
...
dev.off()
```
# SUPPLEMENTARY TABLE 2: Estimate survival time 0 to 19 at 160 mo
```{r}
# Overall
summary(survfit(Surv(fumo, can_dead) ~ 1, data = seer0to19), times=60) # estimates 60 month survival time
...
# In_Medicaid_Flag
results <- data.frame(matrix(ncol=6, nrow=0)) #create the table
columns<-c("Medicaid_Category", "Time (months)", "Survival Percentage",  "LowerCI", "UpperCI", "Age_cat") # label the columns with names
...
# SUPPLEMENTARY TABLE 5: Populate a table with survival by tumor type at 12, 24, 36, 48, and 60 mo
```{r}
limited <- seer0to19 %>%
 filter(can_type %in% c("I. Leukemias, myeloproliferative diseases, and myelodysplastic diseases","II. Lymphomas and reticuloendothelial neoplasms", "III. CNS and miscellaneous intracranial and intraspinal neoplasms", "IV. Neuroblastoma and other peripheral nervous cell tumors", "V. Retinoblastoma", "VI. Renal tumors", "VII. Hepatic tumors", "VIII. Malignant bone tumors", "IX. Soft tissue and other extraosseous sarcomas", "X. Germ cell tumors, trophoblastic tumors, and neoplasms of gonads", "XI. Other malignant epithelial neoplasms and malignant melanomas"))
ct<-unique(limited$can_type)
